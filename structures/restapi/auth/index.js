...
route.get("/callback", async (req, res, next) => {
  ...
  await req.redis.setex(`${req.client.config.redis.prefix}:USER:${userKey}`, 7*24*60*60, JSON.stringify(user));
  await req.redis.setex(`${req.client.config.redis.prefix}:USER:${userKey}:KEYS`, 7*24*60*60, JSON.stringify({refresh_token, access_token}));
...
route.get("/logout", async (req, res, next) => {
  ...
  if(seshkey) {
    await req.redis.setex(`${req.client.config.redis.prefix}:USER:${seshkey}`, 15, null);
    await req.redis.setex(`${req.client.config.redis.prefix}:USER:${seshkey}:KEYS`, 15, null);
  }
