...
route.get('/stats', async (req, res, next) => {
  let redisStats = await req.redis.get(`${req.client.config.redis.prefix}:BOT:STATS`);
...
route.get('/commands', async (req,res,next) => {
  let redisCommands = await req.redis.get(`${req.client.config.redis.prefix}:BOT:COMMANDS`);
